...
var extMetadata = new MenuElement( "command", "Extract Metadata", "at the end of myMetaData" , "xx1D" );
}
extMetadata.onSelect = function () { 
var win = new Window( 'dialog', 'Extract Data' ); 
win.alignChildren=["row","bottom"];
win.g1 = win.add('group');
win.title = win.g1.add('statictext',undefined,'Extract Metadata');
...
win.g100.bu1 = win.g100.add('button',undefined,'Extract Data');
...
var DataFile = File(Folder.desktop +"/Extract Metadata"+time() +".csv");
var csvtab = ",";
}else{
    var DataFile = File(Folder.desktop +"/Extract Metadata"+time() +".txt");
    var csvtab = "\t";
    }
...
var Size = (t.spec.length/1024/1024).toFixed(3)+'mb';
    Line += csvtab + Size;
    }
DataFile.writeln(Line);
    }
DataFile.close();
alert("Extract Completed");
}
}
function time(){
var date = new Date();
